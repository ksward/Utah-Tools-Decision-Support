...
public class KnowledgeEngine extends KnowledgeEngineController {
	...
	public KnowledgeEngine() {
		knowledgeEngineFromDRL("VentilatorPediatricDroolRules00.drl");
		}
	 ...
	 */
	public void fireRules(final VentilatorPediatricDecision ventilatorDecision) {
		...
		session.setGlobal("adultWeightLimit", store.getInt(VentilatorPediatricPreferenceConstants.ADULT_WEIGHT_LIMIT));
		
		// Insert the decision object of the proper type
		session.insert(ventilatorDecision);
	 ...
	 */
	public void testFireRules(final VentilatorPediatricDecision ventilatorDecision) {
		StatefulKnowledgeSession session = kbase.newStatefulKnowledgeSession();

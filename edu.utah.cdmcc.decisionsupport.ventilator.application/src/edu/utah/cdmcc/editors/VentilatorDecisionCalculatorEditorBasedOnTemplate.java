...
import patient.laboratory.controller.VentilatorPatientDecisions;
import patient.laboratory.controller.VentilatorPatientLaboratories;
import ventilator.decision.object.VentilatorDecision;
import ventilator.decision.object.VentilatorFieldsAddedToDomainComposite;
import ventilator.decision.object.VentilatorGetPassiveDecisionDialog;
...
ILaboratoryListener{
	...
	private ClassValidator<VentilatorDecision> salineValidator;
	private InvalidValue[] validationMessages;
	//private IPatientLaboratoryController patientLabController = new VentilatorPatientLaboratories();
...
//		SodiumLaboratoryResult result = patientLabController.retrieveCurrentSodiumLabResult(getPatient());
...
//	}
	...
	private void calculateDecision() {
		...
		if (mode == false && instantiateDecisionObject()) {
			...
			VentilatorGetPassiveDecisionDialog dialog = new VentilatorGetPassiveDecisionDialog(getSite()
					.getShell(), (VentilatorDecision) decision);
			dialog.open();
...
//		}}
	...
	private void fireDroolsRulesEngine() throws RulesEngineException {
		...
		getDecisionEngine().fireRules((VentilatorDecision) decision);
	...
	private boolean instantiateDecisionObject() {
		...
		if (decision == null) {
			...
			decision = new VentilatorDecision(decisionTime);
...
//	}
	...
	private GregorianCalendar populatePreviousObservationTime() {
		IPatientDecisionController patientDecisionController = new VentilatorPatientDecisions();
...
//		}
	...
	private boolean validateHypertonicSalineDecisionWithHibernate() {
		...
		salineValidator = new ClassValidator<VentilatorDecision>(
				VentilatorDecision.class);
		validationMessages = salineValidator.getInvalidValues((VentilatorDecision) decision);

...
import hypertonic.decision.object.GenericFieldsAddedToDomainComposite;
import hypertonic.decision.object.GenericGetPassiveDecisionDialog;
...
import patient.laboratory.controller.GenericPatientDecisions;
import patient.laboratory.controller.GenericPatientLaboratories;
 ...
 * 
 */
public class GenericDecisionCalculatorEditorBasedOnTemplate extends DecisionCalculatorEditorTemplate implements
ILaboratoryListener{
	public KnowledgeEngine decisionEngine;
	private GregorianCalendar decisionTime;
	protected StyledText emptyTrace;
	public static String ID = "edu.utah.cdmcc.decisionsupport.generic.editor.decisionCalculator";
	private static Logger log = Logger.getLogger(GenericDecisionCalculatorEditorBasedOnTemplate.class);
	private GenericFieldsAddedToDomainComposite editorComposite;
	private ClassValidator<GenericDecision> salineValidator;
	private InvalidValue[] validationMessages;
	private IPatientLaboratoryController patientLabController = new GenericPatientLaboratories();
	public GenericDecisionCalculatorEditorBasedOnTemplate() {
		super();
	}
	...
	private void createEditorComposite(final Composite parent) {
		editorComposite = new GenericFieldsAddedToDomainComposite(parent, SWT.NULL);
	}
...
//	}
	...
	private void calculateDecision() {
		...
		if (mode == false && instantiateDecisionObject()) {
			...
			GenericGetPassiveDecisionDialog dialog = new GenericGetPassiveDecisionDialog(getSite()
					.getShell(), (GenericDecision) decision);
			dialog.open();
	...
	private boolean checkValuesKludgeRoutine() {
		...
		if(((GenericDecision) decision).getCurrentHypertonicSalineDripRate() > 10){
		...
		if(((GenericDecision) decision).getMeanArterialPressure() > 200){
			String errorMessage = "Mean arterial pressure may not exceed 200.";
			MessageDialog.openError(null, "Error in decision data entry", errorMessage);
			editorComposite.getMAPText().setFocus();
			return false;
		}
		if (((GenericDecision) decision).getIntracranialPressure() != null){
		if(((GenericDecision) decision).getIntracranialPressure() > 100){
			String errorMessage = "Intracranial pressure may not exceed 100.";
			MessageDialog.openError(null, "Error in decision data entry", errorMessage);
			editorComposite.getICPText().setFocus();
			return false;
		}}
		if (((GenericDecision) decision).getCentralVenousPressure() != null){
		if(((GenericDecision) decision).getCentralVenousPressure() > 25){
	...
	private void fireDroolsRulesEngine() throws RulesEngineException {
		...
		getDecisionEngine().fireRules((GenericDecision) decision);
	...
	private boolean instantiateDecisionObject() {
		...
		if (decision == null) {
			...
			decision = new GenericDecision(decisionTime);
	...
	private void populateDomainSpecificDecisionFieldsFromEditor() {
		if(editorComposite.getCurrentHypertonicSalineDripRateText().getText().length()>0){
					((GenericDecision) decision).setCurrentHypertonicSalineDripRate(Double.valueOf(editorComposite
				.getCurrentHypertonicSalineDripRateText().getText()));
		} 
		if(editorComposite.getCVPText().getText().length()>0){
		((GenericDecision) decision).setCentralVenousPressure(Integer.valueOf(editorComposite.getCVPText()
				.getText()));}
		if(editorComposite.getMAPText().getText().length()>0){
		((GenericDecision) decision).setMeanArterialPressure(Integer.valueOf(editorComposite.getMAPText()
				.getText()));}
		if(editorComposite.getICPText().getText().length()>0){
		((GenericDecision) decision).setIntracranialPressure(Integer.valueOf(editorComposite.getICPText()
				.getText()));}
	}
	...
	private void populateClinicalDecisionFromPatient() {
		...
		((GenericDecision) decision).setPreviousObservationTime(populatePreviousObservationTime());
	}

	private GregorianCalendar populatePreviousObservationTime() {
		IPatientDecisionController patientDecisionController = new GenericPatientDecisions();
	...
	private void populateLabValuesFromPatient() {
		...
		//IPatientLaboratoryController patientLabController = new GenericPatientLaboratories();
		...
		if (recentNA != GenericDecision.NOCURRENTSODIUM) {
			((GenericDecision) decision).setCurrentSodiumValue(recentNA.getNumericResult());
			((GenericDecision) decision).setCurrentSodiumDateTime(recentNA.getTimeOfSpecimenCollection());
		} else {
			//TODO Concerned here because the default blank value is -1 but in the rules we might forget this.
		}
		if (previousNA != GenericDecision.NOPREVIOUSSODIUM) {
			((GenericDecision) decision).setPreviousSodiumValue(previousNA.getNumericResult());
			((GenericDecision) decision).setPreviousSodiumDateTime(previousNA.getTimeOfSpecimenCollection());
		} else{
			
		}
		if (recentOsm != GenericDecision.NOCURRENTOSMOLALITY) {
			((GenericDecision) decision).setCurrentOsmolalityValue(recentOsm.getNumericResult());
			((GenericDecision) decision).setCurrentOsmolalityDateTime(recentOsm.getTimeOfSpecimenCollection());
		} else {
			
		}
		if (previousOsm != GenericDecision.NOPREVIOUSOSMOLALITY) {
			((GenericDecision) decision).setPreviousOsmolalityValue(previousOsm.getNumericResult());
			((GenericDecision) decision).setPreviousOsmolalityDateTime(previousOsm
	...
	private boolean validateHypertonicSalineDecisionWithHibernate() {
		...
		salineValidator = new ClassValidator<GenericDecision>(
				GenericDecision.class);
		validationMessages = salineValidator.getInvalidValues((GenericDecision) decision);

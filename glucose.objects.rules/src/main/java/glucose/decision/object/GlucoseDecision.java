...
//	public static final String GETALLVALIDGLUCOSEDECISIONSBYPATIENT = "getAllValidGlucoseDecisionsByPatient";
	...
	public GlucoseDecision(GregorianCalendar decisionTime, String userAction,
		...
		this.carbohydrateStatus = carbohydrateSource;
		this.recommendedInsulinDripRate = 0.0;
		this.recommendedInsulinBolus = 0.0;
		this.recommendedGlucoseBolus = 0.0;
		this.previousGlucoseConcentration = NOPREVIOUSGLUCOSE;
	...
	/**
	 * @return Returns integer about if there is a carbohydrate intake for the
	 *         patient.
	 */
	@Min(value = 0)
	@Max(value = 3)
	public int getCarbohydrateStatus() {
		return carbohydrateStatus;
	}

	/**
	 * @param carbohydrateSource
	 *            Set int value for whether patient is receiving significant
	 *            carbohydrates or not. The actual choices are going to be an
	 *            enumeration.
	 */
	public void setCarbohydrateStatus(int carbohydrateSource) {
		this.carbohydrateStatus = carbohydrateSource;
	}
...
//	}
	...
	public Double adjustInsulinRate(double targetLow, double targetHigh){
		double gluc_midpoint = (targetHigh + targetLow)/2;
		double g2 = getSerumGlucoseConcentration();
		if (g2 == gluc_midpoint){
			g2 = g2 + 1;
			} 
	...
	//An attempt by KSward to make a peds function
	public Double adjustInsulinRatePed(double targetLow, double targetHigh){
		double gluc_midpoint = (targetHigh + targetLow)/2;
		double g2 = getSerumGlucoseConcentration();
		if (g2 == gluc_midpoint){
			g2 = g2 + 1;
		...
		double dgdt = getGlucoseChangePerHour();
		double desired_rate_of_change = -50 * mf;
		double current_insulin_u = getCurrentInsulinDripRate() * getPatientWeight();
		double adjustment = (((dgdt - desired_rate_of_change)/Math.abs(desired_rate_of_change))
		    *Math.abs(mf))/Math.sqrt(current_insulin_u);
		if (adjustment <= -1) return 0.0;
		double adjusted_insulin = current_insulin_u * (1 + adjustment);
		double adjusted_insulin_u_kg = adjusted_insulin / getPatientWeight();
		return adjusted_insulin_u_kg;
	}
	...
	public void setInsulinMode(String insulinMode) {
		this.insulinMode = insulinMode;
	}

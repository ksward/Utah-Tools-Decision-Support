...
/**
 * GenericDecisionPropertySource is part of the Utah Decision Support Tools.
 * 
 ...
 * 
 */
public class GenericDecisionPropertySource implements IPropertySource2 {
	...
	private GenericDecision decision;

	/**
	 * Creates PropertySource for the GenericDecision object. Called by the
	 * GenericDecision object method getAdaptor().
	 * 
	 * @param decision
	 */
	public GenericDecisionPropertySource(final GenericDecision decision) {
	...
	public Object getPropertyValue(final Object id) {
		...
		if (id.equals(PROPERTY_ICP)){
			if(decision.getIntracranialPressure()== GenericDecision.NOVALUEENTERED){
		...
		if (id.equals(PROPERTY_CVP)){
			if(decision.getCentralVenousPressure()==GenericDecision.NOVALUEENTERED){
				return("No value entered");
			} else {
...
//			if (decision.getPreviousGlucoseConcentration() != GenericDecision.NOPREVIOUSGLUCOSE){

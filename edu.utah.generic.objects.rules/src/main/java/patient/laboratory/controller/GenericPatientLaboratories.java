...
public class GenericPatientLaboratories implements IPatientLaboratoryController {
	static Logger logger = Logger.getLogger(GenericPatientLaboratories.class);
	...
	public SerumOsmolalityLaboratoryResult retrieveCurrentOsmolalityLabResult(Patient patient) {
		...
		if (results.size() == 0) {
			return GenericDecision.NOCURRENTOSMOLALITY;
		} else {
	...
	public SerumSodiumLaboratoryResult retrieveCurrentSodiumLabResult(Patient patient) {
		...
		if (results.size() == 0) {
			return GenericDecision.NOCURRENTSODIUM;
		} else {
	...
	public SerumOsmolalityLaboratoryResult retrievePreviousOsmolalityLabResult(Patient patient) {
		...
		if (results.size() > 1) {
			...
			return GenericDecision.NOPREVIOUSOSMOLALITY;
		}
	}
	...
	public SerumSodiumLaboratoryResult retrievePreviousSodiumLabResult(Patient patient) {
		...
		if (results.size() > 1) {
			...
			return GenericDecision.NOPREVIOUSSODIUM;
		}
	}

...
public class TestGlucoseDecisionAdditionValidationDeletionJUnit4 {
	...
	protected ClassValidator<GlucoseDecision> glucoseValidator;
	...
	public void testDecisionCreation() {
		...
		assertEquals("Previous glucose not correct: ", GlucoseDecision.NOPREVIOUSGLUCOSE, (long) decision.getPreviousGlucoseConcentration());
		assertEquals("Previous observation not correct: ", GlucoseDecision.NOPREVIOUSOBSERVATIONDATE, decision.getPreviousObservationTime());
...
//				GlucoseDecision.VALIDGLUCOSEDECISIONCOUNT).uniqueResult();
...
//	}
	...
	public void testGlucoseTooHighExceptionFires() {
		...
		try {
			decision = new GlucoseDecision(decisionDate, "ACCEPT", 2001, 0, 1);
	...
	public void testGlucoseTooLowExceptionFires() {
		decisionDate = new GregorianCalendar(2005, 9, 25, 14, 55, 55);
		try {
			decision = new GlucoseDecision(decisionDate, "ACCEPT", -1, 0, 1);
		} catch (RuntimeException e) {
			validationMessages = glucoseValidator.getInvalidValues(decision);
...
//		decision = new GlucoseDecision(decisionDate, "ACCEPT", 354, 0, 1);
...
//				.getNamedQuery(GlucoseDecision.VALIDGLUCOSEDECISIONCOUNT).uniqueResult();
...
//	}
	...
	public void setUp() throws Exception {
		...
		glucoseValidator = new ClassValidator<GlucoseDecision>(
				GlucoseDecision.class);
		decisionDate = new GregorianCalendar(2005, 9, 25, 14, 55, 55);
		otherDecision = new ClinicalDecision(decisionDate, "BACKCHART");
